...
public final class GoogleGenAiClient implements WorkerClient {
    private static final Pattern TEXT = Pattern.compile(
            "\\\"text\\\"\\s*:\\s*\\\"((?:\\\\.|[^\\\"\\\\])*)\\\"");
    private static final Pattern PROMPT_TOKENS = Pattern.compile(
            "\\\"promptTokenCount\\\"\\s*:\\s*(\\d+)");
    private static final Pattern OUTPUT_TOKENS = Pattern.compile(
            "\\\"candidatesTokenCount\\\"\\s*:\\s*(\\d+)");
    ...
    public void generate(GenerateRequest request, long timeoutMs, StreamObserver<GenerateResponse> observer) {
        if (!isReady()) {
            observer.onError(Status.UNAVAILABLE.withDescription("Google endpoint is not ready").asRuntimeException());
            return;
        }
        inFlight.incrementAndGet();
        long startedAt = System.nanoTime();
        HttpRequest httpRequest = HttpRequest.newBuilder(URI.create(definition.target()))
                ...
                .whenComplete((response, error) -> {
                    ...
                    if (error != null) {
                        ...
                        observer.onError(Status.UNAVAILABLE
                                .withDescription("Google API request failed")
                                .withCause(error)
                                .asRuntimeException());
                        return;
                    }
                    if (response.statusCode() < 200 || response.statusCode() >= 300) {
                        observer.onError(statusFor(response.statusCode())
                                .withDescription("Google API returned HTTP " + response.statusCode())
                                .asRuntimeException());
                        return;
                    }
                    String output = parseText(response.body());
                    if (output == null) {
                        observer.onError(Status.INTERNAL
                                .withDescription("Google API response did not contain candidate text")
                                .asRuntimeException());
                        return;
                    }
    ...
    private String apiKey() {
        return System.getenv().getOrDefault(definition.apiKeyEnvironmentVariable(), "");
    }

    private String requestBody(GenerateRequest request) {
        int maxTokens = request.getMaxNewTokens() > 0 ? request.getMaxNewTokens() : 256;
        return "{\"contents\":[{\"parts\":[{\"text\":\"" + escape(request.getPrompt())
                + "\"}]}],\"generationConfig\":{\"maxOutputTokens\":" + maxTokens
                + ",\"thinkingConfig\":{\"thinkingLevel\":\"" + thinkingLevel() + "\"}}}";
    }

    private String thinkingLevel() {
        String configured = System.getenv().getOrDefault("GOOGLE_THINKING_LEVEL", "minimal");
        if (!configured.equals("minimal") && !configured.equals("high")) {
            throw new IllegalArgumentException("GOOGLE_THINKING_LEVEL must be minimal or high");
        }
        return configured;
    }

    private static String parseText(String response) {
        Matcher matcher = TEXT.matcher(response);
